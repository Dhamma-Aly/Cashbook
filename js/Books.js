...
function renderLedgerPaginationBar(totalFiltered, totalPages) {
  ...
  if (totalFiltered <= LEDGER_PAGE_SIZE) {
    bar.innerHTML = "";
    return;
  }

  const startIdx = (ledgerPage - 1) * LEDGER_PAGE_SIZE + 1;
  const endIdx = Math.min(ledgerPage * LEDGER_PAGE_SIZE, totalFiltered);
  ...
  bar.innerHTML = `
    <div class="flex justify-between items-center gap-3 bg-[#14110d] border border-amber-900/30 px-4 py-3 rounded-xl">
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-[11px] text-amber-400/80 font-bold px-1.5">${ledgerPage} / ${totalPages}</span>
